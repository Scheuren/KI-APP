...
export default function Level4Page() {
  ...
  const handleInteract = useCallback((zone: 'inspector' | 'accuracy' | 'overfit') => {
    if (zone === 'inspector' && !completedZones.includes('inspector')) {
      setPhase('teaching')
    } else if (zone === 'accuracy' && !completedZones.includes('accuracy')) {
      setPhase('accuracy')
    } else if (zone === 'overfit' && !completedZones.includes('overfit')) {
      setPhase('overfit')
    }
  }, [completedZones])
  ...
  const handleAccuracyComplete = () => {
    setCompletedZones(z => [...z, 'accuracy'])
    setXp(x => x + 10)
    setPhase('explore')
  }
  ...
  const handleQuizComplete = (score: number) => {
    ...
    setXp(x => x + score)
    if (typeof window !== 'undefined') {
      localStorage.setItem('mks_level4_complete', 'true')
      localStorage.setItem('mks_level4_xp', String(puzzleXP + score))
    }
    setPhase('complete')
  }
          ...
          <div className="relative" style={{ width: WORLD_WIDTH, height: WORLD_HEIGHT }}>
            ...
            {phase === 'intro' && (
              <div className="absolute inset-0">
                <DialogBox lines={introDialogues4} onComplete={() => setPhase('explore')} playerCharacter={playerCharacter} playerName={playerName} />
              </div>
            )}
            {phase === 'teaching' && (
              <div className="absolute inset-0">
